...
npm test                           # Vitest：领域、组件、服务端契约、规划（128 项）
npx vitest run tests/server        # 高德适配契约测试（57 项，全部使用伪造 fetch，不访问网络）
npx vitest run tests/workers       # 规划算法与 Worker 协议（21 项）
npm run test:e2e                   # Playwright：桌面 + 移动视口（14 项）
...
4. **周边设施候选 UI**：服务端 `/api/places/nearby` 与归一化、契约测试已实现；界面入口未接入，设计文档未定义对应组件。
