...
按《[省脚力路线卡-需求说明书](./省脚力路线卡-需求说明书.md)》与《[省脚力路线卡-详细设计说明书](./省脚力路线卡-详细设计说明书.md)》实现的响应式网页应用：把选好的景点整理成看得懂、可调整、能导出的“省脚力”路线卡。
...
- **行程编辑与多行程管理（本机）**：添加/排序/跳过/恢复站点、逐段记录步行时间，缺失数据只报已知下界与待补充项；支持新建/切换/复制/重命名/删除多份行程，旧单份行程数据首次打开自动迁移，全部保存在本机浏览器（`localStorage`）。
- **大字路线卡与图片导出**：预览与导出共用同一视图模型，逐页 PNG 在本机生成，可离线查看已采纳的路段时间。
- **附近设施候选检索**：站内按类别（厕所/歇脚处）与半径检索附近候选，逐属性确认后才写入；直线距离仅标注“直线距离，非步行路程”，不参与任何计算。
- **设施事实覆盖保护**：厕所开放、歇脚点座位、台阶有无逐属性核对并保留来源（高德地图搜索/POI id/获取与核对时间）；“确认修改”只写本次实际改动，同值不重写；“已核对”退回“待确认”须点名属性二次确认；没查到不等于“没有”。
- **歇脚点候选一键转休息点**：候选确认后可一键插入路线成为休息点（座位事实与来源随迁移保留，候选坐标随写入带出）；坐标缺失时明确提示“位置待确认，暂不能自动获取步行数据”。
- **绕行步行对比**：休息点行显示“歇脚绕行：比直达多走约 X 分钟”（会话内直达对照，注明按当前步行倍数口径）；对比数据缺失或失败时显示“绕行对比待补充”，不用直线距离估算。
- **开放时间保守解析**：全周统一与分日/多时段文本走严格文法解析，结果参与等待开门/闭门冲突计算；解析不了的文本只保留原文并标“待核对”，不编造时段。
